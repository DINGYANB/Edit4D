...
def build_ebsynth():
    if os.path.exists('FRESCO_src/ebsynth/deps/ebsynth/bin/ebsynth'):
    ...
    if os_str == 'Windows':
        ...
        exe_file = 'FRESCO_src/ebsynth/deps/ebsynth/bin/ebsynth.exe'
    elif os_str == 'Linux':
        cmd = 'bash build-linux-cpu+cuda.sh'
        exe_file = 'FRESCO_src/ebsynth/deps/ebsynth/bin/ebsynth'
    elif os_str == 'Darwin':
        cmd = 'sh build-macos-cpu_only.sh'
        exe_file = 'FRESCO_src/ebsynth/deps/ebsynth/bin/ebsynth.app'
    else:
        print('Cannot recognize OS. Ebsynth installation stopped.')
        return

    os.chdir('FRESCO_src/ebsynth/deps/ebsynth')
    print(cmd)
    os.system(cmd)
    os.chdir('../../../..')
...
def download_hed_ckpt():
    ...
    download(url, 'FRESCO_src/ControlNet/annotator/ckpts')
...
def download_ebsynth_ckpt():
    ...
    if os_str == 'Linux':
        ...
        download(url, 'FRESCO_src/ebsynth/deps/ebsynth/bin')
    elif os_str == 'Windows':
        url = ('https://hf-mirror.com/PKUWilliamYang/Rerender/'
