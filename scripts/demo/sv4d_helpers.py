...
def read_video(
    ...
    print(len(images_v0), '*', images_v0[0].shape)
    return images_v0


def preprocess_edit_image(input_path, remove_bg, n_frames=1, W=576, H=576, output_folder=None, image_frame_ratio=0.917, device="cuda"):
    '''
    对编辑后的第一帧进行预处理, 返回处理后的图像
    '''
    ...
    # Crop the edited image, assume the object is already in the center of the image
    white_thresh = 230
    box_coord = [np.inf, np.inf, 0, 0]
    image_arr = np.array(image)     # [1024, 1024, 3] ~ [0 - 255]
    ...
    print(x, y, w, h)
    print(box_coord[0], box_coord[1], box_coord[2], box_coord[3])
    ...
    preprocessed_image = torch.tensor(rgb).permute(2, 0, 1).unsqueeze(0).to(device)
    preprocessed_image = preprocessed_image * 2.0 - 1.0    # Note !!!
    # print("Preprocessed image:", preprocessed_image)
    print("Preprocessed edited image shape:", preprocessed_image.shape)
    save_path = input_path.split(".png")[0] + "_preprocessed.png"

    return preprocessed_image.float()



def preprocess_video(input_path, remove_bg, output_folder, n_frames=21, W=576, H=576, image_frame_ratio=0.917):
    '''
    对输入的视频或一组图像进行预处理。它的功能包括读取视频帧、去除背景、裁剪图像, 并将结果保存为一个新的视频文件。
    '''
    ...
    if isinstance(input_path, str):
        ...
        images = [Image.fromarray(img_array) for img_array in input_path]
...
def sample_sv3d(
    ...
    if sv3d_model is None:
        ...
        print("Load config from:", model_config)
